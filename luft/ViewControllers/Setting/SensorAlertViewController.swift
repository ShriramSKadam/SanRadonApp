//
//  SensorAlertViewController.swift
//  Luft
//
//  Created by Shriram Subash on 20/12/22.
...
class SensorAlertViewController:UIViewController, LTIntialAGREEDelegate {
    ...
    @IBOutlet weak var radonSwitchAlertEnable: UISwitch!
    @IBOutlet weak var temperatureSwitchAlertEnable: UISwitch!
    @IBOutlet weak var humiditySwitchAlertEnable: UISwitch!
    @IBOutlet weak var airPressureSwitchAlertEnable: UISwitch!
    @IBOutlet weak var eco2SwitchAlertEnable: UISwitch!
    @IBOutlet weak var tvocSwitchAlertEnable: UISwitch!
    ...
    override func viewDidLoad() {
        ...
        self.radonSwitchAlertEnable.isOn = true
        self.radonSwitchAlertEnable.addTarget(self, action: #selector(radonSwitchStatusChanged), for: UIControl.Event.valueChanged)
        debugPrint("Radon switch ON")
        
        self.temperatureSwitchAlertEnable.isOn = true
        self.temperatureSwitchAlertEnable.addTarget(self, action: #selector(temperatureSwitchStatusChanged), for: UIControl.Event.valueChanged)
        debugPrint("Temperature switch ON")
        
        self.humiditySwitchAlertEnable.isOn = true
        self.humiditySwitchAlertEnable.addTarget(self, action: #selector(humiditySwitchStatusChanged), for: UIControl.Event.valueChanged)
        debugPrint("Humidity switch ON")
        
        self.airPressureSwitchAlertEnable.isOn = true
        self.airPressureSwitchAlertEnable.addTarget(self, action: #selector(airPressureSwitchStatusChanged), for: UIControl.Event.valueChanged)
        debugPrint("Air Pressure switch ON")
        
        self.eco2SwitchAlertEnable.isOn = true
        self.eco2SwitchAlertEnable.addTarget(self, action: #selector(eco2SwitchStatusChanged), for: UIControl.Event.valueChanged)
        debugPrint("eCO2 switch ON")
        
        self.tvocSwitchAlertEnable.isOn = true
        self.tvocSwitchAlertEnable.addTarget(self, action: #selector(tvocSwitchStatusChanged), for: UIControl.Event.valueChanged)
        debugPrint("tVOC switch ON")
...
//        }
    ...
    @objc func radonSwitchStatusChanged(mySwitch: UISwitch) {
        if  mySwitch.isOn {
            self.lblRadon.textColor = ThemeManager.currentTheme().labelTextColor
            self.btnRadon.isUserInteractionEnabled = true
            UserDefaults.standard.set(true, forKey: "RadonSwitch")
        }else{
            self.lblRadon.textColor = .gray
            self.btnRadon.isUserInteractionEnabled = false
            UserDefaults.standard.set(false, forKey: "RadonSwitch")
        }
    }
    
    @objc func temperatureSwitchStatusChanged(mySwitch: UISwitch) {
        if  mySwitch.isOn {
            self.lblTemperature.textColor = ThemeManager.currentTheme().labelTextColor
            self.btnTemperature.isUserInteractionEnabled = true
            UserDefaults.standard.set(true, forKey: "TemperatureSwitch")
        }else{
            self.lblTemperature.textColor = .gray
            self.btnTemperature.isUserInteractionEnabled = false
            UserDefaults.standard.set(false, forKey: "TemperatureSwitch")
        }
    }
    
    @objc func humiditySwitchStatusChanged(mySwitch: UISwitch) {
        if  mySwitch.isOn {
            self.lblHumidity.textColor = ThemeManager.currentTheme().labelTextColor
            self.btnHumidity.isUserInteractionEnabled = true
            UserDefaults.standard.set(true, forKey: "HumiditySwitch")
        }else{
            self.lblHumidity.textColor = .gray
            self.btnHumidity.isUserInteractionEnabled = false
            UserDefaults.standard.set(false, forKey: "HumiditySwitch")
        }
    }
    
    @objc func airPressureSwitchStatusChanged(mySwitch: UISwitch) {
        if  mySwitch.isOn {
            self.lblAirPressure.textColor = ThemeManager.currentTheme().labelTextColor
            self.btnAirPressure.isUserInteractionEnabled = true
            UserDefaults.standard.set(true, forKey: "AirPressureSwitch")
        }else{
            self.lblAirPressure.textColor = .gray
            self.btnAirPressure.isUserInteractionEnabled = false
            UserDefaults.standard.set(false, forKey: "AirPressureSwitch")
        }
    }
    
    @objc func eco2SwitchStatusChanged(mySwitch: UISwitch) {
        if  mySwitch.isOn {
            self.lblECO2.textColor = ThemeManager.currentTheme().labelTextColor
            self.btnECO2.isUserInteractionEnabled = true
            UserDefaults.standard.set(true, forKey: "ECO2Switch")
        }else{
            self.lblECO2.textColor = .gray
            self.btnECO2.isUserInteractionEnabled = false
            UserDefaults.standard.set(false, forKey: "ECO2Switch")
        }
    }
    
    @objc func tvocSwitchStatusChanged(mySwitch: UISwitch) {
        if  mySwitch.isOn {
            self.lblTVOC.textColor = ThemeManager.currentTheme().labelTextColor
            self.btnTVOC.isUserInteractionEnabled = true
            UserDefaults.standard.set(true, forKey: "TVOCSwitch")
        }else{
            self.lblTVOC.textColor = .gray
            self.btnTVOC.isUserInteractionEnabled = false
    ...
    func moveToWebView() {
        ...
        webdataView.lblHeader?.text = "Sensor Alert Selection"
        webdataView.strTitle = "Sensor Alert Selection"
    ...
    @objc func btnNextSkipTapped(_sender:UIButton) {
        ...
        mainTabbar.avilableDeviceSharedEMailIsfromAddDeviceThres = self.avilableDeviceSharedEMailIsfromAddDeviceThres
        mainTabbar.isFromInitialSelectedDeviceTokenThres = self.isFromInitialSelectedDeviceTokenThres
        self.navigationController?.pushViewController(mainTabbar, animated: true)
    }
    
}
extension SensorAlertViewController {
